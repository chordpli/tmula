...
빠른 데모는 [`README.md`](README.md)에 있고, 이 문서는 그 다음 단계인 심화·실전 가이드입니다.
...
내가 정의한 행동 그래프를 따라 가상 유저를 사람처럼 움직여서, 진짜 유저를 모으지 않고 내 API의 문제를 찾습니다. 결과는 4종 finding으로 분류됩니다.
...
curl/jq/별도 서버 없이 한 줄로 돌립니다. 바이너리가 in-process 엔진을 띄워 실험을 실행한 뒤 findings를 출력합니다.
...
**CI 게이트** - findings가 있으면 비정상 종료해 파이프라인을 막습니다:
```bash
...
findings 분류 대신 숫자 하나, 즉 이 동시성에서 얼마나 버티는지가 필요할 때 씁니다. 컨트롤 플레인을
거치지 않고 in-process로 바로 돌려 달성 RPS·총요청·지속시간·에러율·트래킹오차·p50/p95/p99를 찍습니다.
플래그 레이아웃은 `tmula run`과 같습니다(시나리오 파일 또는 `--target`+`--get`/`--post`):
```bash
# 시나리오 파일을 80 동시성으로
...
샘플 API의 `product`는 약 2% 404, `cart`는 약 8% 500, `checkout`은 부하를 받으면 포화되지만 부하가 줄면 회복됩니다. tmula는 실제 유저가 겪기 전에 이걸 잡아냅니다.
...
```
> `weight` = 그 간선으로 갈 확률, `dependency: true` = 건너뛸 수 없는 순서(cart 없이 checkout 불가).
> `exit` 같은 template 없는 노드는 이탈 지점으로, 유저 세션이 여기서 끝납니다.
...
```
> 안전장치: `allowlist`에 없는 호스트는 차단합니다. `envClass`는 `dev`/`staging`만 허용하고 `prod`는 잠깁니다.
...
브라우저에서 폼으로 입력합니다. 한 줄이면 됩니다. React UI를 바이너리에
임베드해서 엔진을 띄웁니다:

```bash
( cd server && go run ./examples/sample-api ) &    # (선택) 샘플 API :9000 - 당신 서비스면 생략
...
확인한 뒤 **View HTML report / Compare / share** 링크를 씁니다.

> ⚠️ 그냥 `make build`/`go build`는 플레이스홀더 페이지만 임베드합니다(진짜 UI가 아니라
...
모든 경로는 `/api` 아래에 있습니다. 생성 → 실행 → 관찰 → 공유 순서입니다.
...
2. `templates`를 실제 엔드포인트로 바꾸기. 헤더/페이로드에 per-user 값을 주입할 수 있습니다:
...
```
템플릿이 채우는 값은 두 갈래이고, 받는 경로가 다릅니다.

**① 일반 변수(`{{.item}}` 등) - `vars`로 주입.** 비밀값이 아니므로 JSON RunSpec의 `users` 배열에 그대로 실어 POST합니다:
```json
"users": [
...
**② 자격증명(`{{.token}}`·`{{.subject}}`) - HTTP/JSON RunSpec으로는 주입되지 않습니다.** `domain.Credential.Secret`에 `json:"-"`가 붙어 있어(마스킹, AD-011) 요청 바디가 디코드되는 순간 토큰이 떨어져 나가기 때문입니다. `users[].cred`에 토큰을 넣어 POST해도 그 런은 조용히 익명(미인증)으로 돕니다. 에러조차 나지 않습니다. 비밀값은 설계상 직렬화되지 않습니다.
...
3. `graph`로 실제 유저 흐름을 기술하고 재실행. 이제 findings가 당신 서비스 얘기를 합니다.
...
- **closed**: "동시 50명 고정." 간단하지만 동시성은 내가 박은 숫자입니다.
- **open**: "초당 λ명씩 도착." 동시 접속자 수가 결과로 정해집니다(Little의 법칙 `L = λ × W`).
  이게 실제 트래픽 모양이고, 유기적인 부하를 만드는 길입니다.
...
```
초당 약 278명 도착, 정상상태 동시 약 16,667명. 이 숫자를 워크로드에 그대로 넣습니다.
...
```
> `thinkTime` = 스텝 사이 휴지(자연스러운 페이스). `maxConcurrency` = 백프레셔 상한(넘는 도착은
> drop되며 "수요 > 용량" 신호가 됨). UI에선 Workload model을 `open`으로 바꾸면 같은 필드가 폼으로 나옵니다.
>
> ⚠️ 열린 모델은 단일 노드 전용입니다. `workers`/`aggregateWorkers`와 같이 쓰면 400으로 거부됩니다.
...
가중치로 서로 다른 행동 프로파일을 한 런에 섞습니다. 빠른 파워유저와 느린 구경꾼처럼.
...
```
- `weight` - 도착 점유율(합이 1일 필요 없는 상대값)
- `start` / `maxSteps` / `thinkTime` - 페르소나별 진입점·여정 길이·페이스(생략 시 런 기본값)
- 세션 ID에 페르소나가 태깅되어 점유율을 추적할 수 있음

> 검증: 이름은 유일해야 하고, 가중치는 양수여야 하며, 진입 노드는 그래프에 있어야 합니다. closed 모델에 넣으면 400.
...
여러 머신에 걸쳐 대규모로 돌리는 길입니다. 이건 closed 모델(워커마다 유저 샤드를 고정 실행)입니다.
...
기본 분산은 요청마다 1메시지를 마스터로 스트리밍하므로 수백만 요청에선 이게 병목이 됩니다.
...
```
> 트레이드오프(문서화됨): 퍼센타일 통계는 정확하게 유지되지만, findings는 엔드포인트별/연속실행이
> 아닌 run-wide 신호로 거칠어집니다. 대신 네트워크·메모리가 요청량과 무관하게 일정합니다.
...
현재 스케일에는 두 갈래가 있고, 둘은 서로 다른 축입니다.
...
> open(도착률) + 분산은 현재 미지원입니다(검증에서 거부). 한 런에서 둘을 동시에 쓸 수 없습니다.
...
```
> 분산 closed는 `users` 배열 길이로 워커에 분배합니다. 큰 수는 위처럼 `jq`로 생성하세요.
...
| segments 400 | open 모델에서만 허용되고, 진입 노드가 그래프에 있어야 함 |
