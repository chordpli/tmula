...
| **인증 실행** | 가상 사용자마다 실제 자격 증명을 배정합니다 — 토큰 풀, 실제 로그인, 자체 서명 JWT, 세션 쿠키 | ✅ 동작 |
...
트래픽에 **실제 인증**을 실을 수 있습니다. `auth:` 블록을 붙이면 가상 사용자마다 자격 증명이
배정됩니다 — 사전 발급 토큰 풀, 사용자별 실제 로그인(실행 중 갱신 포함), 서명 키를 보유했을 때의
자체 서명 JWT, 세션 쿠키, 그리고 웹 콘솔의 가이드 모드를 통한 OAuth2 IdP까지. 비밀값은 와이어를
건너지 않습니다:

```yaml
auth:
  strategy: login
  source: { file: users.csv, format: csv }   # 계정마다 한 행 (username,password)
  login:
    flow:
      - id: signin
...
전략 선택, 예제, 결정 표는 매뉴얼의 [인증이 필요한 실행](docs/guide.ko.md#인증이-필요한-실행)에
있습니다.
