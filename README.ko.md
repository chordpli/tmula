...
<p align="center">
  <b>
    실제 사용자 트래픽 시뮬레이터 - 사용자를 직접 모으지 않고도
    실제 사용자가 만날 문제를 찾습니다.
  </b><br>
  API 여정을 명시적인 <i>행동 그래프</i>로 만들고, 그 위에서 가상 사용자를 실행합니다.
  사용자는 실제 사람처럼 움직이고, 정해진 규칙 안에서 경로를 벗어나고,
  특정 엔드포인트로 몰립니다. 그래서 단순 부하 테스트와 수동 테스트가 놓치기 쉬운 문제를
  드러냅니다.
</p>
...
tmula는 검증된 부하 테스트 도구를 대체하려는 도구가 아닙니다. k6, Locust, JMeter,
Gatling, Artillery, nGrinder 같은 도구들은 스크립팅, 시나리오, 분산 실행, 대시보드,
CI까지 폭넓게 지원합니다. **tmula**는 조금 다른 관점에서 시작합니다. 사용자 여정을 명시적인
**행동 그래프**로 모델링하고, 그 그래프 위에서 가상 트래픽을 보내 흐름이 어디서
느려지고, 실패하고, 몰리는지 관찰합니다.

가상 사용자는 여정을 따라가고, 분기하고, 망설이고, 때로는 시나리오를 벗어나며, 많이 쓰는
엔드포인트로 몰립니다. tmula에서 이 여정은 노드 = API 호출, 가중치 엣지 = 전이, 절대 건너뛰지
않는 의존 엣지로 표현됩니다. 문제는 세 가지 모드로 드러납니다.
