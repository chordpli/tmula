...
바로 쓸 수 있는 예제는 [`examples/imports/`](../examples/imports)에 있습니다. `shop.openapi.yaml`, `shop.openapi`의 HAR 짝인 `shop-session.har`, 그리고 `ticketing.openapi.yaml`입니다. 이들은 `http://localhost:9000`을 대상으로 하여 번들된 `server/examples/sample-api`와 맞물립니다.
