...
**tmula**는 실사용자 트래픽 시뮬레이터입니다. 한 엔드포인트에 똑같은 요청을 퍼붓는 대신, *가상 사용자*를 명시적인 **행동 그래프**(behavior graph)를 따라 움직이게 해서 진짜 사람처럼 행동하게 합니다 — 분기하고, 망설이고, 가끔 시나리오를 벗어나고, 인기 있는 엔드포인트로 몰립니다 — 그리고 무엇이 깨졌는지를 **finding**으로 분류합니다. 이 가이드는 [README](../README.md)의 심화 동반 문서입니다. 처음에는 부드럽게 안내하고, 이후에는 철저한 레퍼런스가 되며, 특히 모든 JSON 필드(처음 쓰는 분들이 가장 부담스러워하는 부분)를 꼼꼼히 다룹니다.

처음부터 끝까지 다 읽을 필요는 없습니다. *일단 뭐라도 돌려보고* 싶다면 [실행해 보기](#실행해-보기)와 [웹 콘솔, 필드별 설명](#웹-콘솔-필드별-설명)을 읽으세요. JSON 편집기 앞에서 막히면 [JSON 형식 — 완전한 레퍼런스](#json-형식--완전한-레퍼런스)로 바로 건너뛰면 됩니다. 이 문서의 모든 내용은 소스 코드에 근거합니다 — 필드 이름, 기본값, 검증 메시지는 실제 그대로 인용했습니다.
...
- **엣지**(edge)는 노드 사이의 전이입니다. 각 엣지에는 **weight**(가중치)가 있습니다 — 사용자가 그 엣지를 탈 상대적 확률입니다. 한 노드에서 엔진은 가중치에 비례해 나가는 엣지를 고릅니다.
- **의존 엣지**(dependency edge, `dependency: true`)는 강한 전제 조건입니다: 대상 노드는 이 선행 노드를 *반드시 요구*하며, 사용자의 무작위 이탈이 이를 **절대** 건너뛸 수 없습니다. (shop 데모에서 `cart → checkout`은 의존 엣지입니다: 장바구니 없이는 결제할 수 없습니다.)
...
- **클로즈드 모델**(closed model) — 반복하는 동시 사용자 `N`명의 고정 풀입니다. 단순하지만 동시 실행 수가 설정한 값 그대로입니다. "50명, 항상 50명이 진행 중"이라고 생각하면 됩니다.
- **오픈 모델**(open model) — 사용자 *세션*이 시간에 따라 일정 **비율(rate)**로 도착합니다(예: 초당 새 사용자 200명). 동시 사용자 수는 도착률 × 세션 지속 시간(리틀의 법칙, Little's Law)으로 *자연스럽게 결정*됩니다 — 실제 공개 트래픽이 움직이는 방식 그대로입니다. 공개 서비스에는 이쪽이 현실적인 기본값입니다.

단계 사이에 사용자는 **생각 시간**(think time)만큼 멈춥니다(실제 사람은 행동 사이에 읽고 판단하기 때문에 무작위 지연을 줍니다). 오픈 모델에서는 도착하는 사용자를 **페르소나**(persona, 세그먼트)로 나눌 수도 있습니다 — 가중치가 있는 사용자 유형으로, 각자 자기 시작 노드와 속도를 가집니다 — 그래서 "느린 첫 방문 둘러보기 70%, 빠른 파워 유저 30%" 같은 식으로, 단일한 로봇이 아니라 섞인 집단을 시뮬레이션할 수 있습니다.

### 세 가지 모드

tmula는 서로 겹치는 세 가지 방식으로 문제를 드러냅니다.

- **시나리오 따라가기**(scenario-following) — 현실적이고 분기하는 트래픽 아래에서 정상 경로(happy path)가 버티는가?
- **이탈**(deviation) — 확률적 건너뛰기, 단계 재정렬, 페이로드 변형(의존 엣지는 절대 위반하지 않음)으로 시나리오 밖 버그를 흔들어 냅니다.
- **부하 집중**(load-concentration) — 가상 사용자를 한 API로 몰아넣고 어디서 성능이 떨어지거나 포화되는지 봅니다.

### "finding"이란

**finding**은 tmula가 *클라이언트 쪽*에서 감지한, 분류된 문제입니다(상태 코드, 지연 시간, 오류 패턴 — 서버 계측은 필요 없습니다). 네 가지 종류가 있습니다: `contract`, `availability`, `threshold`, `mutation`, 각각 심각도(severity)를 가집니다. 이것이 핵심입니다: 실행의 결과물은 "실제 트래픽이라면 무엇이 깨졌을지"입니다. 각각이 정확히 어떻게 계산되는지는 [Finding 자세히 보기](#finding-자세히-보기)를 참고하세요.

### 평범한 부하 도구와 무엇이 다른가

평범한 부하 도구는 똑같은 요청으로 URL 하나를 두들기고 처리량 숫자 하나를 알려줍니다. 그러나 *결제가 장바구니가 있고 동시 실행이 높을 때만 500을 낸다*는 사실은 알려줄 수 없습니다. 여정이나 의존 관계를 전혀 모델링하지 않기 때문입니다. tmula는 실제 퍼널을 걷고, 전제 조건을 지키고, 규칙 안에서 이탈하며, 실패를 *종류별로* 분류합니다 — 그래서 숫자 하나가 아니라 버그를 얻습니다.
...
<http://localhost:8080>을 엽니다. Compose 네트워크 안에서 엔진은 예제 API를 **서비스 이름**으로 찾으므로 `localhost`가 아니라 `http://sample-api:9000` / `http://ticketing-api:9100`을 쓰세요. 이유는 [FAQ](#문제-해결--faq)를 참고하세요.
...
> **중요 — 플레이스홀더 UI vs 진짜 UI.** 그냥 `make build` / `go build`는 "run `make web`"이라고 안내하는 *플레이스홀더* 페이지만 내장합니다. 의도된 동작입니다: CLI는 UI 빌드가 필요 없습니다. 진짜 브라우저 콘솔을 얻으려면 `make web`으로 UI를 내장해야 합니다(또는 이미 내장되어 있는 Docker 이미지 / 미리 빌드된 바이너리를 쓰세요). 콘솔이 비어 보이며 `make web`을 실행하라고 한다면 바로 이 이유 때문입니다.

---

## 웹 콘솔, 필드별 설명

콘솔(`make web` 이후 <http://localhost:8080>)에는 세 개의 설정 카드 — **Target**, **Load model**, **Scenario** — 가 있고, 그 다음에 **Run** 버튼과 실시간 보기가 있습니다. 아래에 인용한 도움말 문구는 `web/src/i18n.ts`의 실제 UI 문구입니다. 관련된 곳마다 `?` 도움말 툴팁을 그대로 옮겼습니다.

가장 빠른 시작: **Scenario** 카드에서 **Start from a template**를 누르고 **Branching shop**, **Concert tickets**, **Health check**, **API read flow** 중 하나를 고르세요. 그래프, 템플릿, 시작 노드, 최대 단계가 자동으로 채워집니다(ticketing 프리셋은 Base URL도 `http://localhost:9100`으로 바꿉니다). 그다음 손보고 Run 하면 됩니다.
...
| **Allowlist** | 트래픽이 닿아도 되는 호스트(쉼표 구분) — 실행이 대상 밖으로 새어 나가지 못하게 막는 안전장치. | `localhost`, 또는 Docker에서는 `sample-api` | **콘솔은 Base URL 호스트를 자동으로 추가하지 *않습니다*.** 대상 호스트를 Base URL과 Allowlist *양쪽 모두*에 넣어야 합니다. 그러지 않으면 모든 요청이 차단됩니다(아래 참고). |
...
> **Allowlist 함정(처음 쓰는 사람이 가장 많이 겪는 문제).** 코드상 `buildRunSpec`(`web/src/api.ts`)은 Allowlist 필드를 *오직* 트림하고 분리하기만 합니다 — Base URL 호스트를 대신 넣어 주지 *않습니다*. 그러면 안전 가드가 그 목록에 없는 호스트로 가는 요청을 모두 차단합니다. 그래서 Base URL을 `http://sample-api:9000`으로 두고 Allowlist를 비워 두면(또는 다른 곳을 가리키면) **모든 요청이 실패**하고 실행이 "전부 오류"로 보입니다. 항상 대상 호스트를 두 필드 모두에 넣으세요. (압축 시나리오 *파일* 경로와 CLI는 allowlist를 대상 호스트로 기본 설정해 줍니다 — 하지만 웹 콘솔은 그렇지 않습니다.)
...
| Field | 의미 | 적절한 값 | 주의점 |
|-------|------|-----------|--------|
| **Workload** | `Open`(도착률 기반, 현실적) 또는 `Closed`(반복하는 고정 풀). | 공개 서비스에는 Open | Open은 in-process 전용입니다 — 분산 워커 불가(RunSpec 검증 참고). |
...
| **Personas** | Open 전용, 고급. 가중치 있는 사용자 유형의 JSON 배열, 각자 자기 시작 노드와 속도. 비우면 단일 균일 집단. | 처음에는 빈 값 | [페르소나 / 세그먼트](#페르소나--세그먼트) 참고. |
...
| **Virtual users** | Closed: 풀 크기. Open: 대략적인 상한. | Closed 50; Open은 아무 양수나 | 오픈 실행에서는 *명목상* 값입니다 — 세션은 도착률에서 나옵니다. 그래도 > 0이어야 합니다(실험 검증이 0을 거부). |
...
**Run**을 누르면 콘솔이 [RunSpec](#전체-runspec)을 조립해 POST 합니다. 자동으로 처리하는 두 가지를 알아 두세요: 클로즈드 실행에서는 `users: []`와 `userCount`를 보내고(그래서 거대한 실행도 작은 요청 본문이 됩니다 — 서버가 `u0..uN-1`을 합성), 안전 `rateCap`을 설정한 부하에 맞춰 크기 조정합니다. 이 둘은 UI에서 직접 쓸 일이 없습니다.

---

## JSON 형식 — 완전한 레퍼런스

이 절이 가이드의 핵심입니다. JSON 편집기가 부담스럽다면 이 절을 한 번 읽어 두세요. 그러면 더 이상 무섭지 않습니다 — 각 형식은 작고 규칙적입니다. 아래의 모든 필드는 **타입**, **의미**, **예시**를 적었고, 그 뒤에 완성된 예시 하나와 검증 규칙(서버가 반환하는 정확한 오류 메시지 포함)을 붙였습니다.

> **팁:** 이 전부를 손으로 쓸 일은 거의 없습니다. 웹 **프리셋**과 **Import**가 그래프 + 템플릿을 채워 주며, CLI에서는 [압축 시나리오 YAML](#압축-시나리오-yaml)이 가장 친절한 작성 경로입니다. 원시 RunSpec은 파워 유저와 스크립팅용입니다.
...
**Weight 의미.** 한 노드 안에서 나가는 가중치는 상대적입니다 — 엔진이 정규화합니다. shop 그래프의 `browse → search (0.4)`, `browse → category (0.4)`, `browse → exit (0.2)`는 대략 40% / 40% / 20%를 뜻합니다. (압축 시나리오 경로는 더 엄격한 규칙을 적용합니다: 한 노드의 나가는 전이 가중치 합이 ≤ 1. 손으로 쓴 RunSpec 그래프는 각 가중치가 `>= 0`이고 유한하기만 하면 됩니다.)
...
**경로 안전성 규칙**(`runspec.validateTemplatePath`) — `path`는 반드시:
...
이 규칙들은 템플릿 경로가 요청을 대상 호스트 밖으로 절대 돌리지 못하게 하기 위함입니다. (경로로 *렌더링되는* 변수는 요청 시점에 allowlist 가드가 추가로 잡습니다.) 또한 `method`가 비어 있으면 `api: template "<id>": method is required`.

### 페르소나 / 세그먼트

**오픈 모델 전용.** 콘솔의 "Personas" 편집기와 RunSpec의 `segments` 필드: 도착하는 사용자에서 추출되는, 가중치 있는 행동 프로필의 JSON 배열입니다. 각 재정의는 선택입니다 — `name`과 `weight`만 설정한 세그먼트는 실행 기본값처럼 동작하지만 자기 정체성으로 집계됩니다.
...
완전하고 자기 완결적인 실행 정의 — `POST /api/experiments`의 원시 본문입니다. 스크립팅이나 고급 제어에만 필요합니다. 콘솔과 시나리오 파일이 알아서 만들어 줍니다. 다음이 *실제* 형태입니다(`runspec.RunSpec`과 `web/src/api.ts`가 POST 하는 내용 기준):
...
| `users` | array | 명시적 가상 사용자 풀(클로즈드). 보통 비워 둠 — 서버가 합성하게 둡니다. |
...
> **`virtualUserCount`는 오픈 실행에서도 > 0이어야 합니다.** 이게 사람들을 헷갈리게 합니다. `experiment.params.virtualUserCount`는 워크로드 모델과 무관하게 `> 0`으로 검증됩니다(`experiment: virtualUserCount must be > 0`). 오픈 실행에서는 *명목상* 필드입니다 — 실제 사용자는 도착률에서 나옵니다 — 그래도 양수를 설정해야 합니다. 시나리오 파일 경로는 오픈 실행에서 이를 자동으로 `1`로 설정합니다.
...
가장 친절한 작성 경로입니다. `tmula run scenario.yaml`은 이 압축 문서를 읽고 `Expand`하여 전체 RunSpec으로 만들며, 나머지는 합리적인 기본값으로 채웁니다. YAML *또는* JSON입니다(둘 다 파싱되며 필드 이름은 json 태그와 일치). 모든 블록을 사용하는 완성 예시는 다음과 같습니다.
...
| `allow` | array | 허용 호스트. **기본값: 대상의 호스트**(파일 경로는 웹 콘솔과 달리 이를 *직접* 도출해 줍니다). |
...
`Expand`가 기본값을 채우는 방식도 알아 두면 좋습니다: `flow`에서 그래프와 템플릿을 도출하고(요청을 가진 각 단계는 `t_<id>` 템플릿이 됨), 연속 단계를 가중치 엣지로 연결하며, 넉넉한 `rateCap` `{ maxRps: 10000, maxConcurrency: 1000 }`, `envClass: dev`, `start`를 첫 단계로 설정합니다. 압축 그래프는 더 엄격한 시나리오 규칙으로 검증됩니다(전이 가중치 `[0,1]`, 노드별 나가는 합 ≤ 1, 의존 엣지가 DAG를 형성).
...
시나리오 파일(또는 단일 엔드포인트 플래그)에서 RunSpec을 만들고, 실행한 뒤 — 기본은 in-process, 또는 실행 중인 `--engine` 대상 — finding을 출력합니다.
...
**종료 코드:** `0` 정상 · `1` 오류(또는 failed/killed 실행) · `2` 게이트 아래 finding 감지됨. 게이트는 `--fail-on-findings` 또는 `--fail-on-severity warning`에 대해서는 모든 finding을 셉니다; `--fail-on-severity critical`은 critical만 셉니다. 깨끗하게 완료되지 못한 실행(failed 또는 killed — 예: 타임아웃이나 kill-switch 작동)은 finding과 **무관하게** 비정상 종료하므로, CI 게이트를 조용히 통과하는 일은 절대 없습니다.

플래그 파서는 위치 인자를 루프로 수집하므로 `tmula run scenario.yaml --users 50`과 `tmula run --users 50 scenario.yaml`이 모두 동작합니다.

### `tmula bench` — 용량 프로브

목표 동시 실행 수로 bench 하니스를 SUT에 대해 구동하고 용량 지표(달성 RPS, 지연 백분위수, 추적 오차)를 출력합니다. 컨트롤 플레인이 아니라 bench 하니스를 직접 씁니다.
...
### `tmula init` — 명세에서 시나리오 스캐폴딩

기존 API 설명(OpenAPI 또는 HAR)을 압축 시나리오 파일로 바꿔, 빈 페이지가 아니라 실제 엔드포인트에서 시작하게 합니다.

| Flag | 기본값 | 의미 |
|------|--------|------|
| `--from <file>` | — | **필수.** OpenAPI 또는 HAR 파일. |
| `--format <f>` | auto | `auto` \| `openapi` \| `har`. |
| `--out <file>` | stdout | 시나리오를 쓸 위치. |
| `--target <url>` | — | 대상 base URL 재정의. |
...
finding은 `{ runId, category, severity, evidenceRef, firstSeen, description }`입니다. 네 가지 종류와 세 가지 심각도(`critical`, `warning`, `info`)가 있습니다. 단일 노드 경로는 API별·종류별로 분류합니다 — 나쁜 엔드포인트 하나가 수백 개가 아니라 finding *하나*를 냅니다 — 순서는 mutation → contract → availability → threshold입니다. 각각이 정확히 어떻게 계산되는지는 다음과 같습니다(`internal/obs/finding.go`). 먼저, 곳곳에서 쓰이는 두 술어:
...
> **분산/집계 경로에 대한 참고.** 워커가 샤드를 집계하면(`aggregateWorkers`), finding은 병합된 `Summary`(`FindingsFromSummary`)에서 나오며, 이는 종류별 집계만 보관합니다 — API별 분해도, 순서도 없습니다. 그래서 실행 전체에 대해 작동한 종류당 최대 *하나*의 finding을 내는데, 엔드포인트별 단일 노드 finding보다 일부러 더 거칩니다. 이것이 당신이 선택한 정밀도-규모 맞바꿈입니다. (미묘한 점: Summary의 threshold 오류율/p95는 변형된 것을 포함해 *모든* 관측을 세는 반면, 단일 노드 분류기는 변형 요청을 제외합니다 — 분산 경로가 변형 관측을 운반하지 않기 때문에 오늘은 잠재적입니다.)
...
**지연 히트맵.** 2차원 히스토그램입니다: 행은 지연 구간(낮음 → 높음; 맨 위 구간은 상한 없음, 예: "5s+"), 열은 실행 시작 이후 시간 버킷, 각 셀의 색은 그 구간 × 시간의 요청 수입니다(진할수록 밀도 높음). "지연이 어디로 갔나"를 보는 정석 뷰입니다 — 맨 위에 얇고 뜨거운 꼬리가 있으면 느린 소수가 있다는 뜻입니다.
...
그래프 + 템플릿을 손으로 작성할 일은 거의 없습니다. 임포터가 **OpenAPI** 명세나 **HAR** 기록을 바로 편집 가능한 시나리오(그래프 + 템플릿 + start + maxSteps)로 바꿔 줍니다.

- **웹 콘솔:** Scenario 카드 → **Import from OpenAPI / HAR** → 파일 업로드 또는 텍스트 붙여넣기 → **Import**. 그래프와 템플릿 필드가 채워집니다; 검토하고 Run 하세요. (엔드포인트는 `POST /api/import?format=auto|openapi|har`입니다.)
- **CLI:** `tmula init --from <openapi.yaml|session.har> --out scenario.yaml`.

**형식 감지**는 확장자뿐 아니라 구조적입니다(`detectFormat`): `.har` 이름은 HAR로 강제하고, 그렇지 않으면 문서를 파싱해 키를 살핍니다 — OpenAPI 표지(`openapi` / `swagger` / `paths`)를 먼저 확인하고, 그다음 HAR의 `log.entries`. 덕분에 진짜 브라우저 HAR이 `.har` 이름 없이도 올바르게 가져와집니다.
...
바로 쓸 수 있는 예제는 [`examples/imports/`](../examples/imports)에 있습니다: `shop.openapi.yaml`, `shop.openapi`의 HAR 짝인 `shop-session.har`, 그리고 `ticketing.openapi.yaml`. 이들은 `http://localhost:9000`을 대상으로 하여 번들된 `examples/sample-api`와 맞물립니다.

---

## 인증이 필요한 실행

시뮬레이션 트래픽이 진짜 인증 정보를 운반하게 하려면 **자격 증명 풀**(credential pool)을 붙이세요. 각 클로즈드 가상 사용자(**사용자 인덱스** 기준) 또는 오픈 세션(**세션/도착 인덱스** 기준)에 자격 증명이 배정되며, 항목보다 사용자가 많으면 돌아가며 재사용합니다. 템플릿 헤더에서 참조하세요 — `"Authorization": "Bearer {{.token}}"` — 또는 비민감 주체에는 `{{.subject}}`를 쓰세요.

가장 친절한 방법은 시나리오 파일의 `auth:` 블록입니다.
...
**비밀이 in-process에만 머무는 이유.** 도메인 `Credential.Secret` 필드에는 `json:"-"`가 붙어 있어 비밀이 **절대 직렬화되지 않습니다** — HTTP/SSE/저장소 와이어를 건널 수 없고 영속화되지도 않습니다(`String()`도 로그에서 비밀을 가립니다). 구체적으로:

- `auth:` 블록이 있는 `tmula run`은 비밀이 마샬링될 필요가 절대 없도록 컨트롤 플레인을 **in-process**로(자체 Go API를 통해) 실행합니다. 비인증 경로는 동등성을 위해 여전히 HTTP로 실제 루프백 엔진을 부팅합니다.
...
**제약**(검증 기준): 현재 실행 경로에서는 미리 공급된 `pool` 전략만 동작합니다 — `bootstrap-signup`은 명확한 "not yet supported via this run path (follow-up)" 메시지로 거부됩니다. 자격 증명 풀은 분산 워커와 **결합할 수 없습니다**(워커 팬아웃이 자체 비인증 사용자를 합성하기 때문).
...
**`aggregateWorkers`와 정밀도 맞바꿈.** 기본적으로 각 worker는 모든 요청을 다시 스트리밍하고, master는 실행 길이 기반 availability 감지로 엔드포인트별 finding을 분류합니다(완전 정밀도). `aggregateWorkers: true`이면 각 worker가 자기 샤드 전체를 압축 가능한 작은 **Summary**(카운터 + 메모리 제한 지연 히스토그램)로 접고 master가 그것들을 병합합니다 — 수백만 요청에서도 네트워크와 메모리가 제한되지만, finding이 **실행 전체·종류별**이 됩니다(작동한 종류당 finding 하나, 엔드포인트별 분해 없음, 연속 실패 구간 없음). 요청량이 스트리밍을 압도할 때 쓰고, 엔드포인트별·실행 길이 finding을 원하면 끄세요.

**언제 분산 모드를 써야 하나:** 단일 머신이 필요한 부하를 생성할 수 없고(또한 SUT가 처리할 수 없을) 때만 쓰세요. 오픈 워크로드 모델은 **in-process 전용**임에 유의하세요 — 오픈 실행에 대해 분산 워커는 거부되고(`api: distributed workers are not supported with the open workload model`), 자격 증명 풀도 워커와 함께 거부됩니다.

---

## 안전장치

tmula는 일부러 트래픽을 집중시키므로, 오발이 곧 자초한 장애가 됩니다. 세 가지 가드(`internal/safety`)가 그런 일이 우연히 일어나기 어렵게 만듭니다; 모든 외부 요청은 세 가지를 모두 통과합니다.

- **Allowlist.** 호스트 허용 목록(`TargetEnv.Allowlist`) — 실행이 닿아도 되는 유일한 호스트들. 목록에 없는 호스트로 가는 요청은 차단됩니다(`safety: host "<h>" not in allowlist`). 패턴은 정확히 일치하거나 선행 `*.` 와일드카드(`*.example.com`)입니다. allowlist는 비어 있으면 안 됩니다 — "아무 데나 닿기" 모드는 없습니다.
- **Rate cap.** 강한 상한: `rateCap.maxRps`(토큰 버킷, 버스트는 1초치 비율로 제한)와 `rateCap.maxConcurrency`(진행 중 상한). 둘 다 `> 0`이어야 합니다. 둘 중 하나를 초과하면 대상을 덮치는 대신 그 요청에 `LimitError`가 납니다.
- **Kill switch.** 항상 켜진 **수동** 정지(콘솔의 **Kill run** 버튼), 그리고 선택적 **자동** 작동: 최근 N개 결과에 대한 *롤링* 오류율이 임계값을 넘으면 가드가 스스로 작동합니다(`auto: rolling error rate X over last N exceeded threshold Y`). 자동 작동은 포화를 실제로 관찰할 수 있도록 **기본 비활성화**입니다.
...
이들을 합치면, 실행은 당신이 적지 않은 호스트에 닿을 수 없고, 설정한 비율/동시 실행을 넘을 수 없으며, 언제나 멈출 수 있고, 운영(production)을 실수로 칠 수 없습니다.

---

## 예제 도메인

완전히 실행 가능한 두 데모는 *일부러 심어 둔 버그*의 카탈로그 역할도 합니다. 그래서 tmula를 거기에 겨눴을 때 어떤 finding을 기대해야 하는지 알 수 있습니다. 웹 **프리셋**으로 하나 고르거나(시나리오 *와* 대상을 모두 채웁니다) CLI에서 실행하세요.
...
`hold`의 `409`와 `pay`의 `503`은 그 특정 엔드포인트에 집중됩니다 — 바로 진짜 심어 둔 버그의 패턴입니다(엔드포인트 집중 오류에 대한 FAQ 참고).
...
아닙니다 — 그건 **과부하 / 포화**의 신호이고, 보통 **동시 실행 상한 없이 같은 머신에서** 부하를 *생성*하면서 SUT를 *서빙*하기 때문입니다. 빠른 성공이 p50 ≈ 0을 만들고, 포화된 꼬리가 p95를 초 단위로 밀어 올리며, 타임아웃이 쌓입니다. **Max concurrency**(오픈 모델)를 설정하거나 **Arrival rate**를 낮추거나, 테스트 대상 시스템을 별도 머신으로 옮겨 해결하세요. 이는 SUT의 버그가 아니라 당신 하니스의 측정 아티팩트입니다.

**Q: 오류가 특정 몇몇 엔드포인트에 집중됩니다.**
보통 테스트 대상 시스템의 *진짜* 버그입니다 — 그리고 예제 도메인에서는 일부러 심어 둔 것입니다. ticketing의 `POST /hold` 409(좌석 경쟁)와 `POST /pay` 503(혼잡 시 결제), 또는 shop의 `POST /cart` 500과 `POST /checkout` 성능 저하가 바로 tmula가 드러내려고 만든 엔드포인트 집중 실패입니다. 엔드포인트 집중 오류 → 그 엔드포인트를 보세요; 포화 증상을 동반한 실행 전체 오류 → 당신의 하니스를 보세요(앞 질문).
...
실을 수 없습니다 — 그리고 실행 경로가 이를 거부합니다: 원격 `--engine`에 대한 자격 증명 풀은 비밀이 와이어를 건널 수 없어(`json:"-"`) 거부됩니다. 인증하려면 in-process로 실행하세요(`--engine` 없이 `auth:` 블록과 함께 `tmula run`).
