...
9. [인증이 필요한 실행](#인증이-필요한-실행)
   - [어떤 인증 전략을 쓸까? (여기서 시작)](#어떤-인증-전략을-쓸까-여기서-시작)
   - 전략: [pool](#전략-pool-미리-공급된-자격-증명) · [login](#전략-login-실행-시-토큰-발급) · [bootstrap-signup](#전략-bootstrap-signup-실제-계정-프로비저닝) · [mint](#전략-mint-로컬-jwt-자체-발급) · [exec](#전략-exec-직접-가져온-토큰--탈출구)
   - [세션 쿠키 인증](#세션-쿠키-인증) · [비밀이 in-process에만 머무는 이유](#비밀이-in-process에만-머무는-이유)
   - [웹 OAuth2 가이드 · basic/apiKey import · 보류된 것](#웹-oauth2-가이드--basicapikey-import--보류된-것) · [refresh token은 어떻게 동작하나](#refresh-token은-어떻게-동작하나)
10. [분산 모드](#분산-모드)
11. [안전장치](#안전장치)
12. [예제 도메인](#예제-도메인)
...
- **이탈**(deviation). 단계마다 설정한 확률(`deviationRate`, `0..1`; 콘솔의 **경로 이탈률** 필드)로 사용자가 가중치 경로를 벗어납니다 - 여정을 중간에 포기하거나 가능성 낮은 전이를 탐험합니다. 의존 엣지는 절대 위반하지 않습니다.
- **부하 집중**(load-concentration). 실행 전체를 엔드포인트 하나로 겨눕니다(단일 엔드포인트 `tmula run --get /path`, 또는 노드 하나짜리 그래프). 어디서 성능이 떨어지거나 포화되는지 봅니다.

> **아직 실행에 포함되지 않은 것:** 페이로드 변형(payload mutation), 단계 재정렬(step reordering), 시간에 따라 모양이 변하는 집중 프로파일은 만들어져 있지만 실행 경로에 배선되지 않았습니다 - [README 로드맵](../README.md#roadmap)을 참고하세요. 특히 `mutation` finding 종류는 페이로드 변형을 위해 예약된 것으로, 현재는 발생하지 않습니다.

### "finding"이란

**finding**은 tmula가 클라이언트 쪽에서 감지해 분류한 문제입니다(상태 코드, 지연 시간, 오류 패턴을 보며 서버 계측은 필요 없습니다). 종류는 `contract`, `availability`, `threshold`, `mutation` 네 가지이고 각각 심각도(severity)를 가집니다. 실행의 결과물은 "실제 트래픽이라면 무엇이 깨졌을지"입니다. finding에는 **증거 번들**(evidence bundle) - 재현 좌표를 가진 대표 실패 세션들과 상태 코드·발생 시간 분포 - 이 동봉되며, `tmula reproduce`는 그중 한 세션을 단독으로 재실행해 기능 버그와 부하 의존 버그를 구분합니다. 각각이 어떻게 계산되는지는 [Finding 자세히 보기](#finding-자세히-보기)를 참고하세요.
...
**명령 하나, 설정 제로: `tmula demo`.** 바이너리가 설치되어 있다면(아래 설치 스크립트) 이 한 명령이 전체 루프를 자급자족으로 돌립니다. 심은 버그가 있는 미니 shop을 에페메랄 로컬 포트에 띄우고, 그 shop의 액세스 로그에서 행동 그래프를 *학습*하고, 학습된 트래픽을 다시 그 shop에 재생한 뒤(엔진 + 웹 콘솔은 `--addr`, 기본 `:8080`), finding과 구체적인 다음 단계를 출력합니다. 전체 레퍼런스는 [`tmula demo`](#tmula-demo--한-명령으로-전체-루프)를 참고하세요.

```bash
tmula demo                 # 학습된 트래픽 약 1분, 그다음 finding + 다음 단계
```
...
| **경로 이탈률** (Deviation rate) | 각 단계에서 사용자가 가중치 경로를 벗어날 확률(%) - 다른 경로를 탐험하거나 중도에 포기합니다. 의존 엣지는 절대 위반되지 않습니다. | 시나리오를 그대로 따르려면 `0`; 시나리오 밖 버그를 흔들어 보려면 5-15 | 콘솔은 퍼센트(0-100)를 받아 RunSpec의 `params.deviationRate`(`0..1`)로 매핑합니다. |
...
- **Test login**(버튼 라벨은 패널을 따라갑니다: bootstrap은 **Test signup**, pool/mint는 **Test token**): 실행에 들어가기 전에 인증 설정을 **한 번** 실행해 보는 사전 점검입니다 — 로그인 flow를 한 번 돌리고, pool 항목을 파싱하고, mint 키를 해석해 토큰을 서명합니다(`POST /api/auth/preflight`). HTTP 상태, 토큰이 감지된 위치(본문 키 또는 쿠키), 마스킹된 토큰 접두사를 보여 주므로, 안 채운 비밀이나 실패한 캡처가 401투성이 실행이 아니라 클릭 한 번으로 드러납니다.
...
- `{{.username}}` / `{{.password}}`. **login flow 본문 전용** — 지금 로그인 중인 가상 사용자의 자격 증명 행(`subject` = 사용자명, `token` = 비밀번호). [서로 다른 N개 계정으로 로그인](#서로-다른-n개-계정으로-로그인-행이-곧-자격-증명) 참고.
- `{{.userIndex}}`. 가상 사용자의 번호로, **인증 템플릿** — login/signup flow 본문, mint의 `subject`/`claims`, exec의 `command`/`env`, `usersPattern` — 에서 `user{{.userIndex}}` 같은 예측 가능한 사용자별 정체성에 씁니다.
...
| Field | Type | 의미 |
|-------|------|------|
| `experiment` | object | 실행과 실행 시 파라미터에 이름을 답니다. `name` 필수; `params.virtualUserCount`는 `> 0`; `params.deviationRate`는 `[0,1]`(단계별 이탈 확률 - [세 가지 모드](#세-가지-모드) 참고); `params.authStrategy`는 `pool` 또는 `bootstrap-signup`. |
...
| `metrics` | object | 선택. `{ "prometheusUrl": string, "queries": [{ "name", "query" }] }` - 실행 후 서버 메트릭을 보고서에 병치. 호스트는 allowlist에 있어야 합니다. |
| `findings` | object | 선택. `{ "errorRate": number, "p95LatencyMs": number, "availabilityStreak": int }` - 관측을 finding으로 분류하는 임계값을 조정합니다. 생략(또는 `0`)된 필드는 기본값을 유지합니다. [임계값 조정](#임계값-조정-findings-블록) 참고. |
...
| `deviationRate` | number | 각 단계에서 사용자가 가중치 경로를 벗어날 확률(`0..1`) - 여정을 포기하거나 가능성 낮은 전이를 탐험합니다. 의존 엣지는 절대 위반되지 않습니다. 기본값 `0`(모든 사용자가 정상 경로를 따름). 범위를 벗어나면 `scenariofile: deviationRate <v> out of range [0,1]`. |
...
| `metrics` | object | 선택. 실행 후 Prometheus 시계열을 보고서에 병치([서버 메트릭 병치](#서버-메트릭-병치-prometheus) 참고). |
| `findings` | object | 선택적 finding 임계값(아래). |
...
**`auth` 필드.** [인증이 필요한 실행](#인증이-필요한-실행) 참고. `strategy`는 `pool`(기본), `login`, `bootstrap-signup`, `mint`, `exec` 중 하나입니다. `pool`일 때: `users`는 `{ subject, token }` 목록이거나, `source: { file, env, format }` 또는 `usersPattern: { subject, token, count }`를 씁니다(셋은 상호 배타적). `login`일 때: `login.flow`, `login.capture.token`(선택; 비우면 자동 감지), `login.capture.subject`(선택), `login.scope`(`per-user` | `shared`). `bootstrap-signup`일 때: `signup.flow`, `signup.capture.token`, `signup.teardown`(선택), `keepAccounts`(bool). `mint`일 때: [mint 블록](#전략-mint-로컬-jwt-자체-발급). `exec`일 때: [exec 블록](#전략-exec-직접-가져온-토큰--탈출구)(명시적 opt-in 게이트).

**`findings` 필드**(모든 필드 선택; `0`이거나 생략된 값은 기본값을 유지)

| Field | Type | 의미 / 기본값 |
|-------|------|---------------|
| `errorRate` | number | 전체 오류율이 이보다 크면 → `threshold` finding. 기본값 `0.2`. `[0,1]` 범위여야 합니다(아니면 `scenariofile: findings: errorRate <v> out of range [0,1]`). |
| `p95LatencyMs` | number | 전체 p95 지연(ms)이 이보다 크면 → `threshold` finding. 기본값 `0` = p95 게이트 **비활성** 유지. 음수면 안 됩니다. |
| `availabilityStreak` | int | 한 API에서 이만큼 연속 실패하면 → `availability` finding. 기본값 `5`. 음수면 안 됩니다. |

같은 블록(필드 이름 동일)이 RunSpec의 `findings` 필드입니다. [임계값 조정](#임계값-조정-findings-블록)을 참고하세요.
...
`graph`를 쓰면 `start`가 필수이고(그래프에 있는 노드여야 함), 템플릿을 가리키는 모든 노드의 템플릿이 `templates`에 있어야 하며, `open` / `segments` / `auth` / `users` / `deviationRate` / `findings` 등 나머지 블록은 flow 형식과 똑같이 동작합니다. 검증도 같은 엄격한 시나리오 규칙을 통과해야 합니다.
...
### `tmula demo` - 한 명령으로 전체 루프

첫 실행 경험입니다. 내장된 버그 있는 shop을 띄우고, 트래픽에서 행동 그래프를 학습하고, 그 트래픽을 재생하고, finding을 읽습니다 - 시나리오 파일도, 두 번째 터미널도 필요 없습니다.
...
| Flag | 기본값 | 의미 |
|------|--------|------|
| `--addr <addr>` | `:8080` | 데모의 엔진 + 웹 콘솔 수신 주소. 포트가 점유되어 있으면 이 플래그를 가리키는 에러로 즉시 실패합니다(예: `--addr :8081`). |
| `--duration <dur>` | `60s` | 학습된 트래픽이 계속 도착하는 시간. 양수여야 합니다. |
| `--no-browser` | false | 웹 콘솔을 브라우저로 열지 않습니다. (열기는 macOS `open` / Linux `xdg-open`을 사용하고, 그 외 플랫폼에서는 경고 후 출력된 URL이 폴백입니다.) |

단계별 동작(`[1/4]`..`[4/4]` 단계 출력이 이를 그대로 보여 줍니다):

1. **데모 shop 기동** - 심은 버그(간헐적으로 실패하는 cart, 부하에 따라 성능이 떨어지는 checkout, 드물게 깨진 상품 링크)가 있는 미니 스토어를 *에페메랄* 루프백 포트에 띄웁니다. 그래서 충돌할 수 있는 포트는 엔진의 `--addr` 하나뿐입니다.
2. **행동 그래프 학습** - shop에 번들된 액세스 로그에서, `tmula init`이 쓰는 것과 같은 [액세스 로그 학습기](#액세스-로그에서-그래프-학습하기)로 엔드포인트, 분기 가중치, 이탈, think time, 그리고 오픈 워크로드 제안을 학습합니다. 페이싱은 데모 창에 맞게 압축되지만(심은 버그가 확실히 드러날 만큼의 세션 수 보장) 학습된 그래프와 가중치는 그대로입니다.
3. **로컬 엔진 + 내장 웹 콘솔 기동** - `--addr`에 띄우고 학습된 트래픽을 shop에 실행합니다. 런 스펙은 모든 `tmula run`이 쓰는 것과 같은 시나리오 경로로 확장됩니다 - allowlist는 shop의 호스트만으로 기본 설정되고, [안전장치](#안전장치)가 전부 적용됩니다. 런은 라이브 트레이싱이 켜진 채 생성되고 브라우저는 `/?run=<run-id>`로 열리므로, UI가 내장된 콘솔이라면 설정 폼 대신 그 런의 라이브 뷰 - 트래픽 플로우 맵과 실시간 지표 - 에 바로 붙습니다.
4. **결과 출력** - 창이 닫히면 finding이 담긴 표준 런 요약을 출력하고, 이어서 **Next steps**를 출력합니다: 첫 finding에 바로 붙여넣을 수 있는 `tmula reproduce` 명령, 그 런의 `report.html` URL, 콘솔 URL, 그리고 같은 루프를 내 서비스에 겨누는 `tmula init --from <내 액세스 로그>` / `tmula run` 짝.

요약 후에도 데모는 (엔진과 shop 모두) **계속 떠 있어** 그 명령들이 그대로 동작합니다. **Ctrl-C**는 어느 단계에서든 - 런 도중이라도 - 둘 다 정상 종료하고 `0`으로 끝납니다. 실패는 `1`로 끝납니다. **finding 게이트는 일부러 없습니다**: 데모가 버그를 *찾는* 것이 성공이므로, `tmula run`과 달리 `--fail-on-findings`가 없고 finding은 종료 코드에 영향을 주지 않습니다.

> **내장 UI 참고.** 브라우저 콘솔 페이지는 UI가 내장된 바이너리(미리 빌드된/설치 스크립트 바이너리, Docker 이미지, 또는 `make web` 빌드)에서만 진짜 콘솔입니다. 그냥 `go build`로도 데모는 끝까지 동작하며 - 터미널 요약과 `report.html` URL이 결과 표면입니다 - 콘솔 루트는 플레이스홀더 페이지가 뜹니다. UI가 내장된 빌드에서는 데모가 여는 `/?run=<run-id>` 링크가 콘솔을 그 런의 라이브 뷰(플로우 맵 + 실시간 지표, 런이 끝나면 보고서)에 바로 붙입니다. 내장 UI가 없으면 데모 자신의 런은 터미널 요약과 링크된 HTML 보고서로 확인합니다.
...
| `--baseline <run-id>` | - | 회귀 게이트: 이 실행의 finding을 `--engine`(이 형태에서는 필수)에서 가져온 베이스라인 런과 비교합니다. 베이스라인 대비 **새로운** finding이 있을 때만 exit `3`. [베이스라인 게이트](#베이스라인-게이트--회귀에만-실패하기) 참고. |
| `--baseline-file <file>` | - | 저장된 보고서 JSON(이전 `tmula run --json` 출력, 예: CI 아티팩트)에 대한 회귀 게이트. `--baseline` / `--baseline-file`은 둘 중 하나만 쓸 수 있습니다. |
| `--known-issues <file>` | - | known-issues YAML: 일치하는 **새로운** finding은 항목의 `expires` 날짜(`YYYY-MM-DD`)까지 베이스라인 게이트에서 억제됩니다. `--baseline` 또는 `--baseline-file`이 필요합니다. |
...
**종료 코드**(우선순위 순서 - 뒤의 게이트는 앞의 게이트를 모두 통과했을 때만 평가됩니다):

| 코드 | 의미 | 발생 조건 |
|------|------|-----------|
| `0` | 깨끗함 | 실행이 완료되고 켜진 게이트 중 어느 것도 작동하지 않음. 베이스라인이 있을 때 *persisting* finding만 있으면 여전히 `0`입니다 - 이번 변경이 깨뜨린 것이 아니기 때문입니다. |
| `1` | 오류, 또는 failed/killed 실행 | finding과 무관하게 항상 비정상 종료(예: 타임아웃이나 kill-switch 작동). 깨진 실행이 CI를 조용히 통과하는 일이 없습니다. |
| `2` | finding 감지됨 | `--fail-on-findings` / `--fail-on-severity`: 절대 게이트. 베이스라인 유무와 무관하게 (조건에 맞는) 모든 finding을 세므로, `--baseline`과 함께 써도 의미가 유지됩니다. |
| `3` | 베이스라인 대비 새로운 finding | `--baseline` / `--baseline-file`: 회귀 게이트. known-issue 억제를 적용한 뒤 새로운 finding에만 실패합니다. |

절대 게이트는 `--fail-on-findings` 또는 `--fail-on-severity warning`에 대해서는 모든 finding을 세고, `--fail-on-severity critical`은 critical만 셉니다.

플래그 파서는 위치 인자를 루프로 수집하므로 `tmula run scenario.yaml --users 50`과 `tmula run --users 50 scenario.yaml`이 모두 동작합니다.

### 베이스라인 게이트 - 회귀에만 실패하기

`--fail-on-findings`는 절대적입니다. 몇 주째 깨져 있던 finding이라도 잡(job)을 실패시킵니다. **베이스라인 게이트**는 대신 비교합니다. finding은 안정적인 정체성 `(category, evidenceRef)`로 키가 매겨지고(실행마다 달라지는 숫자는 키에 없습니다) 베이스라인 런과 diff되므로, CI는 *이번* 변경이 깨뜨린 것에만 빨개집니다. 베이스라인을 지정하는 방법은 두 가지입니다.

- `--baseline-file report.json` - 저장된 `tmula run --json` 출력. CI에서 자연스러운 형태입니다: main 브랜치 잡이 보고서를 아티팩트로 올리고, PR 잡이 내려받아 그에 대해 게이트합니다.
- `--baseline <run-id> --engine <url>` - 장기 실행 엔진의 실행 이력에서 베이스라인을 가져옵니다(`GET /api/runs/{id}/report`). in-process 실행은 빈 이력으로 시작하므로 이 형태에는 `--engine`이 필수입니다.

판정은 네 버킷입니다: **new**(게이트를 실패시키는 유일한 버킷, exit `3`), **resolved**, **persisting**(베이스라인에서도 깨져 있던 것 - 보고만 되고 실패시키지 않음), **suppressed**. 판정은 보고서 뒤에 출력되고, 마크다운 `--summary`에 **"Baseline gate"** 섹션으로 덧붙습니다(표 하나, new가 먼저, suppressed 행에는 reason과 만료일 주석).

**Known issues.** `--known-issues <file>`은 수용하기로 한 *새로운* finding을 억제합니다. 파일은 YAML 리스트이고 **모든 필드가 필수**라서, 익명이거나 영원한 억제는 있을 수 없습니다.

```yaml
- category: contract
  evidenceRef: checkout            # finding의 정체성, 보고서가 출력하는 그대로
  reason: known cart-service hiccup, tracked in SHOP-123
  expires: 2026-07-31              # YYYY-MM-DD - 이 억제가 유효한 마지막 (UTC) 날짜
```

매칭은 `(category, evidenceRef)` 정확 일치이며(글로빙 없음), 알 수 없는 YAML 필드는 거부됩니다 - 오타가 억제를 조용히 무력화할 수 없습니다. `expires`는 재검토 날짜를 강제합니다. 만료일 당일까지는 여전히 억제되고, 그 다음 (UTC) 날부터 항목이 동작을 멈춥니다 - 해당 finding은 다시 **new**가 되어 게이트를 실패시키고, 만료된 항목은 stderr(`--json` 모드에서도 살아남습니다)와 요약에 경고로 표시되어, 파일에서 썩는 대신 재검토되거나 삭제되도록 합니다.
...
**액션에서 회귀에만 게이트하기.** 액션의 입력은 CLI의 베이스라인 게이트를 그대로 비춥니다.

- `baseline-file`: 이전 `tmula run --json` 보고서의 경로(자연스러운 형태: main 브랜치 잡이 보고서를 아티팩트로 올리고, PR 잡이 내려받습니다). 설정하면 회귀 게이트가 켜집니다 - 베이스라인 대비 **새로운** finding이 있을 때만 exit `3`으로 잡이 실패합니다. `fail-on: none`과 함께 쓰면 순수하게 회귀에만 게이트합니다.
- `known-issues`: known-issues YAML 경로로, `--known-issues`로 전달됩니다. `baseline-file`과 함께일 때만 유효합니다(단독 사용은 CLI가 런 전에 일찍 거부합니다).
- `pr-comment: 'true'`: `pull_request` 이벤트에서 **Baseline gate** 판정 표를 PR 코멘트로 게시하고, 재실행 시 *같은 코멘트를 제자리에서 수정*합니다(`comment`는 실행마다 새 코멘트를 쌓는 것과 다릅니다). 베이스라인이 없으면 대신 전체 런 요약을 게시합니다. 워크플로의 `pull-requests: write` 권한이 필요하며, 포크 PR은 읽기 전용 토큰을 받으므로 거기서는 경고와 함께 코멘트를 건너뜁니다 - 잡의 빨강/초록은 언제나 런 자체의 종료 코드를 따르고, 코멘트 배관을 따르지 않습니다.

```yaml
      - uses: actions/download-artifact@v4
        with: { name: tmula-baseline, path: baseline }   # main이 저장한 보고서
      - uses: chordpli/tmula@main
        with:
          scenario: tests/journey.yaml
          target: http://localhost:9000
          fail-on: none                        # 회귀에만 게이트
          baseline-file: baseline/report.json  # 새로운 finding에만 exit 3
          known-issues: tests/known-issues.yaml
          pr-comment: true                     # 게이트 판정을 PR 코멘트로 upsert
```

액션 없이도 맨 `tmula run`이 GitHub Actions와 협력합니다. `GITHUB_STEP_SUMMARY`가 설정돼 있으면 마크다운 요약이 자동으로 덧붙고, 실행이 실패했거나 게이트가 작동했을 때도 요약은 기록됩니다 - 빨간 잡이 종료 코드가 아니라 *무엇이 깨졌는지*로 바로 이어집니다. 회귀에만 게이트하려면 `--baseline-file`을 추가하세요(새로운 finding에 exit `3` - [베이스라인 게이트](#베이스라인-게이트--회귀에만-실패하기) 참고). 판정 표는 같은 스텝 요약에 덧붙습니다.

### `tmula reproduce` - 진짜 버그인가, 부하 아티팩트인가?

엔드포인트별 finding의 [증거 번들](#증거-번들-evidence)에는 세션들의 **시드 좌표**(런 시드 + 유저 인덱스 = 세션 시드)가 담겨 있습니다. `tmula reproduce`는 finding의 첫 증거 세션을 **단독으로** - 세션 하나, 동시 부하 없음, 같은 결정론적 walk - 여러 번 재실행하고, 실패가 얼마나 자주 재발하는지로 근본 원인을 분류합니다.

| 판정 (`rootCauseClass`) | 의미 |
|--------------------------|------|
| `functional` | **모든** 시도에서 재현됨: 부하가 필요 없는 실패 - 평범한 기능 버그일 가능성이 높습니다. |
| `load-dependent` | **어느** 시도에서도 재현 안 됨: 원래의 동시성이나 포화가 있어야 나타날 가능성이 높습니다. |
| `flaky` | 일부 시도에서만 재현됨 - `--attempts`를 늘려 다시 돌리거나 대상을 직접 살펴보세요. |

| Flag | 기본값 | 의미 |
|------|--------|------|
| `--engine <url>` | - | **필수.** 그 실행을 돌렸고 아직 보유 중인 엔진. |
| `--run <run-id>` | - | **필수.** finding을 재실행할 런 id. |
| `--finding <sel>` | - | **필수.** `category/evidenceRef`(예: `contract/checkout` - 보고서가 출력하는 키 그대로) 또는 그 런의 finding 목록에서 1부터 세는 인덱스. |
| `--attempts <n>` | 3 | 단독 재실행 횟수(1-20). |
| `--json` | false | 표 대신 원시 reproduce JSON 출력. |
| `--timeout <dur>` | 2m | 재실행을 기다리는 최대 시간. |
...
출력에는 세션과 시드 산술, 원래의 실패 경로, 시도마다의 단계별 상태 코드(finding의 신호를 낸 단계는 `!` 표시), 그리고 판정이 표시됩니다. 판정은 저장된 finding에 `rootCauseClass`로도 새겨지므로(라이브 캐시와 Store 양쪽) 나중에 다시 가져온 보고서에도 이미 끝난 분류가 보입니다(`(root cause: load-dependent)`).

재실행은 원래 실행의 제약을 그대로 지킵니다. 안전 가드를 재구성해 적용하고(allowlist, rate cap, `prod-locked` 대상은 거부), 페르소나 세션은 해당 세그먼트의 시작 노드·max-steps 재정의를 적용하며, 자격 증명 풀 실행은 같은 인덱스로 같은 자격 증명을 다시 받습니다.

**한계.** 재실행은 엔진에서 일어납니다(`POST /api/runs/{id}/reproduce`). 시드 좌표는 그 런의 spec 옆에서만 의미가 있는데, spec은 엔진 메모리에만 살기 때문입니다. 재시작된 엔진이나 축출된 런은 `410`으로 답합니다 - 나중에 reproduce할 계획이라면 장기 실행 엔진에 대고 실행하세요. 요약 유래(run-wide) finding은 세션 좌표가 없고, `mutation` finding은 재실행이 만들지 않는 입력이 필요하므로 둘 다 거부됩니다(`422`). 그리고 판정은 **증명이 아니라 신호**입니다. 재실행이 재현하는 것은 세션의 *트래픽 구성*(같은 시드, 같은 walk)이지, 원래의 타이밍·동시성·대상 상태가 아닙니다 - 엔진은 이 한계 문구를 모든 결과에 함께 돌려줍니다.
...
`run` / `reproduce` / `init` / `bench` / `demo` 없이 호출하면 장기 실행 엔진이 시작됩니다.
...
finding은 `{ runId, category, severity, evidenceRef, firstSeen, description, count }`입니다(`count`는 finding 뒤에 있는 발생 횟수이며, 비율을 설명에 담는 threshold finding에서는 생략됩니다). 선택적으로 `evidence` 번들([아래](#증거-번들-evidence))과 - `tmula reproduce`를 거친 뒤에는 - `rootCauseClass`를 가집니다. `evidenceRef`는 실행마다 달라지는 숫자가 들어 있지 않은 finding의 *안정적인 정체성*입니다 - 엔드포인트별 finding은 API(노드) id, 실행 전체 threshold finding은 `error-rate` / `p95-latency`, 요약 유래 집계는 `run-wide` - 그래서 같은 문제는 실행이 달라도 같은 키를 가지며, 베이스라인 게이트가 diff하는 기준이자 known-issues 항목이 지목하는 이름입니다. 종류는 네 가지, 심각도는 세 가지(`critical`, `warning`, `info`)입니다. 단일 노드 경로는 API별·종류별로 분류하므로, 나쁜 엔드포인트 하나가 수백 개가 아니라 finding *하나*를 냅니다. 순서는 mutation → contract → availability → threshold입니다. 각각의 계산 방식은 다음과 같습니다(`server/internal/obs/finding.go`). 먼저, 곳곳에서 쓰이는 두 술어입니다.
...
| **mutation** | warning | **변형된** 입력이 **failed**됨(`mutationSignal`). 변형 테스트는 입력을 일부러 실패시키므로 이는 정보 제공용입니다. API당 finding 하나: *"mutated input surfaced N error(s) on `<api>`."* **예약됨:** 페이로드 변형은 아직 실행 경로에 배선되지 않아([로드맵](../README.md#roadmap)) 어떤 요청도 변형으로 표시되지 않으며, 이 종류는 현재 발생하지 않습니다. |
...
임계값은 실행마다 설정할 수 있습니다. RunSpec과 압축 시나리오 파일 모두 선택적 `findings` 블록(`obs.FindingConfig`)을 받습니다.

```yaml
findings:
  errorRate: 0.1          # 기본값 0.2
  p95LatencyMs: 800       # 기본값 0 (게이트 비활성)
  availabilityStreak: 3   # 기본값 5
```

| Field | 의미 / 기본값 |
|-------|---------------|
| `errorRate` | 전체 오류율이 이보다 크면 → threshold finding. 기본값 `0.2`. |
| `p95LatencyMs` | 전체 p95 지연이 이 ms보다 크면 → threshold finding. 기본값 `0`이면 p95 게이트는 **비활성** 유지. |
| `availabilityStreak` | 한 API에서 이만큼 연속 실패하면 → availability finding. 기본값 `5`. |

모든 필드는 선택이며, `0`(또는 생략)인 필드는 기본값으로 폴백합니다 - 블록이 없는 스펙은 블록이 생기기 전과 정확히 같게 분류됩니다. 여기서 비대칭이 하나 생깁니다. 이 블록으로 오류율·availability 게이트를 *조이거나 풀* 수는 있지만 끌 수는 없고(`0`은 "끔"이 아니라 "기본값"), p95 게이트는 직접 설정하지 않는 한 꺼져 있습니다. 내부적으로 블록은 `ClassifyConfig`(`ErrorRateThreshold` / `P95LatencyMs` / `AvailabilityRun`)로 해석되며, *해석된* 값이 `0`이면 그 게이트가 비활성화됩니다. 블록은 모든 경로에 적용됩니다 - 단일 노드, 분산 스트리밍, 분산 요약(분류는 master에서 일어나므로 워커로 옮길 것이 없습니다).
...
finding은 *무엇이* 깨졌는지를 답합니다. finding에 붙는 **증거 번들**은 *누가, 언제 그것을 맞았고, 어떻게 다시 볼 수 있는지*를 답합니다. 번들은 분류 시점에 한 번, 분류기가 센 것과 같은 관측에서 응축되므로 - 증거가 자기가 뒷받침하는 finding과 어긋날 수 없습니다 - 설계상 크기가 제한됩니다. 뒤에서 몇 개의 요청이 실패했든 finding은 작게 유지됩니다. 와이어(보고서 JSON과 Store)에서는 finding의 선택적 `evidence` 필드입니다.

| Field | 의미 |
|-------|------|
| `vus` | 최대 **대표 세션 5개**: 가장 이른 발생들(문제가 처음 드러난 곳)과 나머지 중 가장 느린 것들(최악의 경우). p95 threshold finding의 후보는 *게이트를 넘은* 요청들입니다 - 그 신호는 실패가 아니라 느림이기 때문입니다. |
| `vus[].vu` | 세션 ID - 그 세션이 모든 요청에 실어 보낸 `X-Tmula-Session-ID` 헤더 값 그대로입니다. 대상 서버 로그를 이 값으로 grep 하면 그 여정 하나를 추적할 수 있습니다. |
| `vus[].seed`, `vus[].userIndex` | 재현 좌표: 런 시드 + `userIndex` = 세션의 walk 시드(`seed`). 인덱스는 클로즈드 모델에서는 풀 인덱스, 오픈 모델에서는 도착 번호, 분산 샤드에서는 전역 유저 인덱스입니다. `tmula reproduce`가 재실행에 쓰는 값입니다. |
| `vus[].persona` | 세션이 추출된 세그먼트(페르소나 믹스가 없는 실행에서는 생략). |
| `vus[].path` | 세션이 실패 요청까지 걸어간 노드 순서(실패 요청 포함). (분산 스트림은 요청별 경로를 운반하지 않으므로 거기서는 비어 있습니다.) |
| `vus[].statusCode` / `latencyMs` / `errorClass` / `ts` | 문제를 드러낸 요청. 상태 `0`은 전송 수준 실패를 뜻하며, 그때는 `errorClass`가 신호입니다. |
| `timeBuckets` | finding의 발생을 관측된 실행 창의 고정 4분위(`0-25%` … `75-100%`)에 분배한 것 - "램프업 초반"과 "소크 후반"을 구분하기에 충분합니다. |
| `statusCounts` | 대표 세션만이 아니라 *모든* 발생의 상태 코드 집계. |

웹 콘솔은 번들을 finding별 접이식 패널로 렌더링하고, 독립 HTML 보고서에도 같은 섹션이 있습니다. 둘 다 공유된(마스킹된) 보고서에서도 살아남습니다. 와이어 이름 `vus`/`vu`는 - 필드 *이름*이 민감해 보이면("session" 포함) 가리는 PII 마스커를 - 이 합성 식별자들이 온전히 통과하도록 일부러 고른 것입니다.

증거를 붙이지 않는 생산자가 둘 있습니다: 요약 유래 finding(`aggregateWorkers`는 설계상 요청별 데이터를 접어 버립니다 - 위 참고 참조)과 번들이 생기기 전에 영속화된 finding입니다. 증거 없는 finding은 이전과 똑같이 렌더링되고, `tmula reproduce`는 이를 거부합니다.
...
**HTML 보고서 & 비교.** 모든 실행에는 독립 실행형 서버 렌더링 **HTML 보고서**(`View full HTML report`)와 회귀를 찾기 위한 실행 간 **비교** 뷰(`Compare with previous run`)가 있습니다. 웹 보고서와 HTML 보고서 모두에서 각 finding은 [증거 패널](#증거-번들-evidence)로 펼쳐집니다: 대표 세션들(서버 로그를 grep 할 `X-Tmula-Session-ID` 값과 `tmula reproduce`가 재실행에 쓰는 시드 좌표 포함), 상태 코드 분포, 발생 시간 분포, 그리고 reproduce가 기록해 둔 `rootCauseClass` 판정(있는 경우).

### 서버 메트릭 병치 (Prometheus)

클라이언트 측 관측은 "무엇이 깨졌는지"까지만 말해 줍니다. 실행에 `metrics:` 블록을 붙이면 **실행이 끝난 뒤 그 시간 창에 대해** 지정한 PromQL 쿼리를 Prometheus에서 가져와, 서버가 본 것(CPU, DB 커넥션, 큐 깊이…)을 클라이언트 지표 옆에 나란히 보여 줍니다. checkout 5xx가 치솟은 시각에 커넥션 풀이 고갈됐는지를 한 화면에서 읽는 용도입니다.
...
- **어디에 보이나.** 웹 보고서(시리즈별 스파크라인 + min/last/max), 독립 HTML 보고서(표), 그리고 `--summary` 마크다운(표).
- **opt-in이고 절대 실행을 실패시키지 않습니다.** Prometheus가 죽었거나 쿼리가 틀렸으면 보고서에 메모로 남고, 성공한 쿼리의 시리즈는 그대로 표시됩니다.
- **allowlist를 따릅니다.** 엔진이 닿는 다른 모든 호스트처럼 Prometheus 호스트도 allowlist에 있어야 합니다.
- **라이브 보고서 전용.** 시리즈는 영속화되지 않으므로 재시작/축출 후 재구성된 보고서에는 없습니다.
- 쿼리당 시리즈 최대 5개, 실행 창을 약 60 스텝으로 샘플링합니다(과대 쿼리가 보고서를 삼키지 않게).

원시 RunSpec에서는 `metrics: { "prometheusUrl": ..., "queries": [...] }` 필드입니다.
...
- **입력 - 내용으로 자동 감지되는 6개 포맷.** Apache/nginx **combined 형식**; 범용 **JSON lines**(한 줄에 객체 하나; `time`/`ts`/`timestamp`, `method`+`path` 또는 `request`, `remote_addr`, `user_agent` 등 흔한 키 철자); **AWS ALB** 액세스 로그; **CloudFront 표준 로그**(탭 구분 W3C 포맷 - 파서가 `#Fields` 헤더가 선언한 컬럼 순서를 그대로 따릅니다); **Caddy** 구조화 JSON; **Traefik** JSON 액세스 로그. 감지(`importer.DetectAccessLogFormat`)는 앞쪽 비공백 라인 몇 개를 프로브하므로, 로그 로테이션으로 첫 줄이 잘린 파일도 감지됩니다.
...
**커버리지 리포트 - 학습기가 남긴 것과 버린 것.** 캡이 걸렸거나 잡음 섞인 임포트가 조용히 "전체 커버리지"인 척해서는 안 되므로, 학습기는 자기 통계를 보고합니다: 사용한 요청 수, 스킵한 줄 수, 세션 수, 고유 클라이언트 수, 접힌 엔드포인트 수, 그리고 최대 10건의 *파싱 실패 샘플*(줄 번호, 줄의 앞 120자, 사유). 그래서 반쯤 깨진 실제 로그는 커버리지가 부분적인 *이유*까지 설명합니다. 설계상 걸러지는 줄(정적 에셋, 여정과 무관한 메서드)은 카운트만 되고 샘플링되지 않습니다. 보이는 곳:

- `tmula init`은 요약 노트를 stderr에 출력합니다(`learned N endpoint(s) from N request(s) across N session(s)…; skipped N unusable line(s)`).
- `POST /api/import`는 통계를 옵셔널 `stats` 객체로 반환할 수 있습니다(배선된 임포터가 통계를 만들지 않으면 생략되므로, 기존 클라이언트는 stats 이전 응답 형태를 그대로 봅니다).
- 웹 콘솔은 그래프 프리뷰 위에 **임포트 커버리지** 패널을 렌더링합니다: 사용/스킵/세션/클라이언트/접힘 요약, 한 줄이라도 스킵됐으면 경고 톤("이 임포트는 캡처된 트래픽의 일부만 반영합니다"), 그리고 서버가 보냈다면 건너뛴 줄 샘플 표.
- **사용 가능한 요청이 0건**이면 임포트가 실패하고, 에러 메시지에 첫 샘플의 줄 번호와 사유가 담깁니다 - 에러 텍스트만으로 조치할 수 있습니다.

**변수 승격(Go API).** 기본적으로 접힌 `{id}` 엔드포인트는 가장 많이 관찰된 단일 경로에 고정됩니다. Go API는 대신 그 세그먼트를 템플릿 변수로 *승격*할 수 있습니다. `importer.FromAccessLogWithOptions`에 `PromoteVariables`를 주면 `/product/{{.product_id}}` 같은 경로(표준 `{{.var}}` [템플릿 보간](#api-템플릿))를 내고, 각 변수의 관측 값 풀(최빈순, `MaxVariableSamples` 캡, 기본 5)을 통계에 보고합니다. 같은 이름의 변수는 엔드포인트 간 풀을 공유합니다 - `/product/{id}`와 `/product/{id}/reviews`는 같은 `product_id` 값 풀에서 뽑습니다. **기본은 off**입니다. 승격된 시나리오는 호출자가 그 풀을 가상 사용자의 `vars`에 시딩해야 렌더링되고, 기본 구체 경로는 그대로 실행되기 때문입니다.

학습기의 노브 - 포맷 힌트, 노드 캡(기본 30; 음수면 캡 해제), 세션 갭(기본 30분), 변수 승격 - 는 Go API의 `importer.AccessLogOptions` / `importer.FromAccessLogWithOptions`에 있습니다. CLI와 웹 임포트는 기본값(자동 감지 포함)을 사용합니다.
...
| 가진 것 | 전략 | 현실성 | 준비 비용 | IdP 부하 | 분산 워커 | 웹 콘솔 경로 |
|---------|------|--------|-----------|----------|-----------|---------------|
| **토큰 뭉치**(사전 발급 bearer 토큰 / API 키) | [pool](#전략-pool-미리-공급된-자격-증명) | 중간 — 정적 토큰, 발급 트래픽 없음 | 최저 — 붙여넣거나 파일 지정 | 없음 | ✅ 외부 `source`(file/env 참조)로 가능 | **이미 토큰이 있어요** |
| **로그인 엔드포인트 + 계정** | [login](#전략-login-실행-시-토큰-발급) | 높음 — 모든 토큰이 실제로 발급됨 | 낮음 — flow 한 단계 + 자격 증명 | prewarm은 병렬이되 동시 16개 이하로 제한 | ❌ in-process 전용 | **로그인해서 토큰을 받아요** |
| **JWT 서명 키**(자체 발급 토큰) | [mint](#전략-mint-로컬-jwt-자체-발급) | 높음 — 사용자별 진짜 JWT, 로그인 트래픽 없음 | 중간 — 키 참조 + 클레임 | 없음 | ✅ 키 참조만 실림 | **토큰을 로컬에서 서명**(Advanced) |
| **OAuth2 IdP** | [웹 OAuth2 가이드](#웹-oauth2-가이드--basicapikey-import--보류된-것)가 조립하는 login | 높음 | 낮음 — 질문 두 개 | prewarm 제한(≤ 16) | ❌ in-process 전용 | **OAuth2 서비스예요** |
| **토큰을 출력하는 CLI뿐** | [exec](#전략-exec-직접-가져온-토큰--탈출구) | 명령에 따라 다름 | 중간 — `--allow-exec` opt-in | 없음(명령이 IdP를 부르지 않는 한) | ❌ in-process 전용 | **명령으로 토큰 가져오기**(Advanced) |

**목표**로 바로 찾기:

- **Auth0 client-credentials(머신 토큰)** → 웹 [OAuth2 가이드 모드](#웹-oauth2-가이드--basicapikey-import--보류된-것)에서 "클라이언트 키로"를 선택 — 또는 [client_credentials 예제](#client_credentials-공유-머신-토큰).
- **5만 사용자 CSV로 로그인** → [login + `source` 행](#서로-다른-n개-계정으로-로그인-행이-곧-자격-증명) (행 하나 = 계정 하나).
- **로그인이 본문 토큰 대신 세션 쿠키를 줌** → [세션 쿠키 인증](#세션-쿠키-인증).
- **IdP 없이 사용자별 JWT** → [mint](#전략-mint-로컬-jwt-자체-발급) (서명 키를 직접 보유해야 합니다).
- **issuer URL만 있음** → OAuth2 가이드의 issuer 디스커버리(**Fetch endpoints**가 `/.well-known/openid-configuration`에서 토큰 URL을 채워 줍니다).
...
auth:
  ...
  login:
    ...
    capture:
      token: tok          # 선택: {{.token}}이 될 추출 변수 (비우거나 생략하면 자동 감지)
      subject: uid        # 선택: {{.subject}}가 될 변수
    scope: per-user       # per-user(기본) | shared(client_credentials)
```

tmula는 가상 사용자마다(`per-user`) 또는 모든 세션이 공유하는 방식으로(`shared`) 로그인 플로를 실행해 토큰을 캡처하고 `{{.token}}`으로 전송합니다. 실행 중 401이 발생하면 한 번 갱신하고(도출 가능하면 실제 refresh token 교환, 아니면 재로그인 — [refresh token은 어떻게 동작하나](#refresh-token은-어떻게-동작하나) 참고) 재시도하며, 그 갱신 트래픽은 finding에서 제외됩니다. 로그인 실행은 **in-process 전용**으로, 원격 `--engine`과 분산 워커에서는 거부됩니다.

> **로그인 prewarm과 IdP.** 사용자별 로그인은 실행 전에 한꺼번에(prewarm) 병렬로 수행되지만, 실행의 `RateCap.MaxConcurrency`를 상한으로 **최대 동시 16개**로 제한됩니다 — 10,000명 실행이 IdP를 상대로 한 로그인 부하 테스트가 되지 않게 하기 위해서입니다. 그래도 IdP가 rate limit을 걸면 [prewarm 중 429 FAQ](#문제-해결--faq)를 참고하세요.

#### 서로 다른 N개 계정으로 로그인 (행이 곧 자격 증명)

위 블록은 **한** 정체성으로 로그인합니다. 가상 사용자마다 *다른* 계정으로 로그인하려면 자격 증명 **행**을 추가하세요 — login 전략에서 행은 로그인 *입력*입니다: `subject` = 사용자명, `token` = 비밀번호. 가상 사용자 *i*는 행 *i*로 로그인하며(순환), 로그인 본문은 `{{.username}}` / `{{.password}}`로 행을 참조합니다.

```yaml
auth:
  strategy: login
  source:
    file: users.csv         # subject,token 행 → username,password
    format: csv
  login:
    flow:
      - id: signin
        request: POST /auth/token
        body: '{"username":"{{.username}}","password":"{{.password}}"}'
    # capture 생략: 토큰은 응답에서 자동 감지됩니다
```

파일이 없다면 [`usersPattern`](#파일-없이-계정-패턴으로-생성-userspattern)이 행을 대신 생성합니다 — 그 `subject`/`token` 템플릿이 여기서 username/password가 되므로(`user{{.userIndex}}` / `pw-{{.userIndex}}`), "패턴으로 만든 50,000개 계정이 각자 실제로 로그인"이 YAML 세 줄로 끝납니다.

#### client_credentials (공유 머신 토큰)

서비스 주체(서버 간 통신)라면 실행 전체가 정체성 하나를 씁니다: **폼 인코딩** `grant_type=client_credentials` 교환에 `scope: shared`를 붙이면 tmula가 한 번 로그인하고 모든 세션이 그 토큰을 재사용합니다.
...
IdP가 토큰에 스코프를 요구하면 `&scope=read+write`를 붙이세요. 추가 파라미터를 요구하는 IdP도 있습니다 — **Auth0는 `audience`가 필수**입니다(`&audience=https://api.example.com`). 없으면 토큰 대신 오류가 돌아옵니다.
...
**저장되는 것도 있습니다: login flow 본문.** `json:"-"` 보장이 지키는 것은 *캡처되거나 발급된* 자격 증명 비밀입니다. flow 본문에 직접 적은 비밀은 여기에 해당하지 **않습니다**: `client_credentials`의 client_secret, 붙여넣은 refresh token, username/password 로그인 본문은 다른 템플릿 본문과 마찬가지로 실행 스펙의 일부입니다 — RunSpec에 실려 다니고, 실행 동안 엔진 메모리에 살며, 컨트롤 플레인에 저장된 스펙에서 보입니다(웹 OAuth2 가이드도 Client secret 필드 옆에서 같은 내용을 안내합니다). 피해 반경을 줄이세요: client_credentials에는 **일회용 테스트 클라이언트**를 쓰고, 사용자별 비밀번호는 `--auth-source env:VAR`나 외부 `source`로 파일 밖에 두거나, [`usersPattern`](#파일-없이-계정-패턴으로-생성-userspattern)으로 생성하세요(비밀 템플릿은 Expand 시 실체화되고, 실체화된 비밀은 `json:"-"`로 마스킹됩니다).
...
- **basic auth / apiKey import.** OpenAPI/Swagger를 import하면 http `basic`(→ `Authorization: Basic {{basicAuth .subject .token}}` 헤더 + username/password 풀), `apiKey` in query(→ 경로에 `?name={{.token|urlquery}}`), `apiKey` in cookie(→ `Cookie: name={{.token}}`)가 자동 도출됩니다. `REPLACE_ME`만 채우면 실행됩니다 — auth 블록에 `REPLACE_ME` 리터럴이 남아 있으면 Expand가 해당 필드명을 짚어 주는 오류로 **거부**하므로, 안 채운 비밀은 실행 중 401 대신 즉시 실패합니다. `openIdConnect` 스킴은 discovery URL을 advisory로 알려 주며, 웹은 그 값을 OAuth2 가이드의 토큰 URL로 안내합니다.
...
**자격 증명**이 어떻게 다뤄지는지 — 무엇이 절대 직렬화되지 않고, 무엇이 실행 스펙에 *실리는지*(client_secret이나 비밀번호 같은 login flow 본문) — 는 [비밀이 in-process에만 머무는 이유](#비밀이-in-process에만-머무는-이유)와 그 "저장되는 것도 있습니다" 주의를 보세요. 또한 [exec 전략](#전략-exec-직접-가져온-토큰--탈출구)의 명령 송신은 이 안전장치 밖에서, 자체 `--allow-exec` opt-in 뒤에서 동작한다는 점도 기억하세요.
...
`--fail-on-findings`(또는 `--fail-on-severity`)는 finding이 감지되면 일부러 `2`로 종료합니다. 게이트가 작동하는 것입니다. `3`은 [베이스라인 게이트](#베이스라인-게이트--회귀에만-실패하기)가 베이스라인 대비 *새로운* finding을 찾았다는 뜻입니다(known-issue 억제 적용 후). 대신 `1`이 났다면 실제 오류이거나 failed/killed 실행(예: 타임아웃이나 kill-switch 작동)이며, 이는 finding과 무관하게 항상 비정상 종료합니다. `0`은 깨끗함을 뜻하고, 베이스라인이 있을 때는 "persisting finding만 있음"도 포함합니다.
...
**Q: 인증 실행의 모든 요청이 401입니다.**
순서대로 점검하세요. (1) 웹 콘솔이라면 먼저 **Test login**을 누르세요 — 인증 설정을 한 번 실행해 상태, 토큰이 감지된 위치, 마스킹된 접두사를 보여 주므로, 깨진 로그인은 클릭 한 번이면 드러납니다. (2) auth 블록에 남은 `REPLACE_ME` 리터럴: 시나리오 Expand가 이제 해당 필드명을 짚어 주는 오류와 함께 **거부**하므로, 실행이 시작조차 됐다면 이건 아닙니다 — 다만 수정 후에는 다시 확인하세요. (3) `{{.token}}`이 어떤 템플릿에도 배선되지 않음: 풀은 스스로 아무것도 주입하지 않습니다. 스텝의 `headers` 블록에서 참조해야 합니다(`"Authorization": "Bearer {{.token}}"` — 점 포함). (4) 로그인이 토큰을 캡처하지 못함: 특이한 응답 형태에는 명시적 `auth.login.capture.token`이 필요하고, 쿠키 기반 세션에는 `Cookie: session={{.token}}` 헤더가 필요합니다([세션 쿠키 인증](#세션-쿠키-인증) 참고).

**Q: 로그인 prewarm 중에 IdP가 rate limit(429)을 겁니다.**
사용자별 로그인 prewarm은 병렬이지만 실행의 `rateCap.maxConcurrency`를 상한으로 동시 16개 이하로 제한됩니다 — 먼저 `rateCap.maxConcurrency`를 낮춰 prewarm 버스트를 늦추세요. 그래도 IdP가 막으면 `scope: shared`(모든 세션이 client_credentials 토큰 하나를 공유)로 바꾸거나, IdP를 아예 경로에서 빼세요: 서명 키를 보유했다면 [mint](#전략-mint-로컬-jwt-자체-발급), 아니면 사전 발급 [pool](#전략-pool-미리-공급된-자격-증명)입니다.

**Q: 실행이 잘 되다가 N분 뒤부터 실패하기 시작합니다.**
부하 버그가 아니라 토큰 수명 문제입니다. mint JWT는 `mint.ttl` 뒤에 만료됩니다 — 실행 시간보다 길게 잡으세요. login 토큰은 401에서 갱신됩니다(실패 요청당 갱신 1회 + 재시도 1회; [refresh token은 어떻게 동작하나](#refresh-token은-어떻게-동작하나) 참고) — 다만 IdP가 사용 시 refresh token을 회전시키는데 붙여넣은 토큰을 다른 곳에서 재사용했다면 회전 체인이 끊겨 갱신이 실패하기 시작합니다. access token만 있는 풀(OAuth2 가이드의 "access token만 있어요")은 갱신 경로가 아예 없습니다 — 토큰 수명보다 긴 실행에는 refresh token을 쓰세요.

**Q: mint로 만든 JWT가 거부됩니다(401 / invalid signature).**
흔한 원인은 셋입니다. (1) HS256의 `secretEncoding`이 틀림 — 같은 키 본문도 `raw`/`base64`/`base64url`에 따라 다르게 읽힙니다. 서비스가 secret을 디코딩하는 방식과 맞추세요. (2) 서비스가 내가 가진 키로 검증하지 않음: 관리형 IdP(Auth0/Cognito/Firebase — 모든 openIdConnect issuer)는 서명 키를 자기가 쥐고 있어, 자체 발급 토큰은 절대 검증을 통과하지 못합니다. 대신 [login](#전략-login-실행-시-토큰-발급)이나 [웹 OAuth2 가이드](#웹-oauth2-가이드--basicapikey-import--보류된-것)를 쓰세요(콘솔이 바로 이 경우를 경고합니다). (3) 시계 오차 — 내 머신에서는 유효한데 서버에서는 "아직 유효하지 않음": `mint.leeway`를 설정하고 `ttl`을 실행 길이와 비교하세요.
